...
class Main {
    ...
    private static void dfs(int[][] graph, int[][] visited, int row, int col, int rowLength, int colLength) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if ((-1 < nextRow && nextRow < rowLength) && (-1 < nextCol && nextCol < colLength)) {
                ...
                if (graph[nextRow][nextCol] == 1 && visited[nextRow][nextCol] == 0) {
                    dfs(graph, visited, nextRow, nextCol, rowLength, colLength);
                }
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < T; i++) {
            ...
            int rowLength = Integer.parseInt(info.nextToken());
            int colLength = Integer.parseInt(info.nextToken());
            ...
            for (int j = 0; j < rowLength; j++) {
                for (int k = 0; k < colLength; k++) {
                    if (graph[j][k] == 1 && visited[j][k] == 0) {
                        count++;
                        dfs(graph, visited, j, k, rowLength, colLength);
                    }
                }
            }
