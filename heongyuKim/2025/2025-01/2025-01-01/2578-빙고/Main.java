...
public class Main {
    ...
    static int checkBingoRow(int[][] bingoBoard) {
        int rowBingo = 0;
        for (int r = 0; r < 5; r++) {
            if (Arrays.stream(bingoBoard[r]).allMatch(value -> value == 0)) {
                rowBingo++;
            }
        }
    ...
    static int checkBingoCol(int[][] bingoBoard) {
        ...
        for (int c = 0; c < 5; c++) {
            int col = c;
            boolean isBingo = IntStream.range(0, 5).allMatch(r -> bingoBoard[r][col] == 0);
    ...
    static int checkBingoDiagonal(int[][] bingoBoard) {
        ...
        int[] rightDownArr = new int[5];
        int[] leftDownArr = new int[5];

        // 오른쪽 아래로 내려가는 대각선
        for (int r = 0; r < 5; r++) {
            rightDownArr[r] = bingoBoard[r][r];
        }

        if (Arrays.equals(rightDownArr, compareArr)) {
            diagonalBingo++;
        }

        // 왼쪽 아래로 내려가는 대각선
        for (int l = 0; l < 5; l++) {
            leftDownArr[l] = bingoBoard[l][4 - l];
        }

        if (Arrays.equals(leftDownArr, compareArr)) {
            diagonalBingo++;
        }
