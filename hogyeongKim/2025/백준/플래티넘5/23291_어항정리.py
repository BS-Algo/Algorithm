...
def stack_and_fold():
    ...
    while len(fish[0]) < len(fish):
        ...
        rotated = []
        for col in zip(*top):
            rotated.append(list(col)[::-1])
        
        # 쌓아야 하는 어항 위에 적재
        for i in range(min(len(rotated), len(bottom))):
            bottom[i].extend(rotated[i])
...
def adjust_fish():
    ...
    for i in range(len(fish)):
        for j in range(len(fish[i])):
            for dx, dy in dt:
                nx, ny = i + dx, j + dy
                if 0 <= nx < len(fish) and 0 <= ny < len(fish[nx]) and fish[i][j] > fish[nx][ny]:
...
def get_difference():
    ...
    if isinstance(fish[0], list):
        ...
        new_fish = fish
    return max(new_fish) - min(new_fish)

while get_difference() > k:
    cnt += 1
